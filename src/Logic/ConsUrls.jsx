export const catalogoGet = "http://192.168.0.195:80/api/Catalogo/Listas/";
export const catalogoDelete = "http://192.168.0.195:80/api/Codigos/Eliminar/";
export const catalogoDetalles =
  "http://192.168.0.195:80/api/Catalogo/Detalles/";
export const catalogoAgregar =
  "http://192.168.0.195:80/api/Codigos/RegistrodeCodigos";
export const catalogoModificar = "http://192.168.0.195/api/Codigos/Actualizar";
export const listNames = "http://192.168.0.195:80/api/Almacen/Nombres";
export const listNamesId = "http://192.168.0.195:80/api/Almacen/Agregar/";
export const listNamesPost = "http://192.168.0.195:80/api/Almacen/Envio/";
export const almacenGet = "http://192.168.0.195:80/api/Almacen/ListaAlmacen";
export const almacenDelete = "http://192.168.0.195:80/api/Almacen/Eliminar/";
export const almacenEditar = "http://192.168.0.195:80/api/Almacen/Editar/";
export const proveedorGet = "http://192.168.0.195:80/api/Proveedor/Listas/";
export const proveedorPost = "http://192.168.0.195:80/api/Proveedor/Agregar/";
export const proveedorPut = "http://192.168.0.195:80/api/Proveedor/Editar/";
export const proveedorDelete =
  "http://192.168.0.195:80/api/Proveedor/Eliminar/";
export const proveedorAgregar =
  "http://192.168.0.195:80/api/Proveedor/Agregar/";
